    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            <div class="bg-white p-4 py-10 rounded-lg">
                ...
                <form action="{{ route('rfid.pairing.update', ['pairId' => $firstReader->pair_id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="md:flex md:justify-between md:gap-5">
                        <div class="mb-5 md:flex-grow">
                            ...
                            @php
                                $selectedReader1 = old('reader_1_id') ?? $firstReader->reader_id;
                            @endphp
                            <select name="reader_1_id" id="reader_1_id"
                                ...
                                @foreach ($readers as $reader)
                                    <option value="{{ $reader->id }}"
                                        {{ $selectedReader1 == (int) $reader->id ? 'selected' : '' }}>{{ $reader->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-10 md:mb-5 md:flex-grow md:mr-7">
                            ...
                            @php
                                $selectedReader1 = old('reader_1_position') ?? $firstReader->reader_position;
                            @endphp
                        ...
                        <div class="mb-5 md:flex-grow md:ml-7">
                            ...
                            @php
                                $selectedReader2 = old('reader_2_id') ?? $secondReader->reader_id;
                            @endphp
                        ...
                        <div class="mb-5 md:flex-grow">
                            ...
                            @php
                                $selectedReader2 = old('reader_2_position') ?? $secondReader->reader_position;
                            @endphp
