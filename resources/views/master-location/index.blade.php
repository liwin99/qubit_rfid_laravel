    ...
    <x-slot name="header">
        ...
        <div class="my-6" x-data="{ open: false }">
            ...
            <fieldset class="border rounded-md p-2 mt-1 hidden" :class="{ 'block': open, 'hidden': !open }">
                <form action="{{ route('master.location.index') }}">
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            <div class="text-right">
                <a href="{{ route('master.location.create') }}" class="inline-block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-auto px-5 py-2.5 text-center">
                    Create New
                </a>
            </div>

            @if (count($masterLocations))
                        ...
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                            <tr>
                                <th scope="col" class="px-6 py-3">
                                    <a
                                        href="{{ route('master.location.index', array_merge(request()->query(), ['sort_by' => 'id'], $get_params)) }}">
                                        <span>Id</span>
                                ...
                                <th scope="col" class="px-6 py-3">
                                    <a
                                        href="{{ route('master.location.index', array_merge(request()->query(), ['sort_by' => 'name'], $get_params)) }}">
                                        <span>Name</span>
                            ...
                            @foreach ($masterLocations as $masterLocation)
                                <tr class="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <a href="{{ route('master.location.edit', ['masterLocation' => $masterLocation]) }}"
                                            class="font-medium text-blue-600 hover:underline">Edit</a>
                                        <form
                                            action="{{ route('master.location.destroy', ['masterLocation' => $masterLocation]) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                ...
                <!-- Pagination links -->
                <div class="mt-4">
                    {{ $masterLocations->appends($_GET)->links() }}
                </div>
            @else
